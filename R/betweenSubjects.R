...
betweenSubjectMPT <- function(
  ...
  if (!inherits(model1, c("betaMPT", "traitMPT", "simpleMPT")) ||
    ...
    stop(
      "The two models 'model1' and 'model2' must be (hierarchical) MPT models",
  ...
  if (!par1 %in% model1$mptInfo$thetaUnique |
    ...
    stop(
      "MPT parameters not found in models. Must be in the set:\n  ",
      paste0(union(
        model1$mptInfo$thetaUnique,
        model2$mptInfo$thetaUnique
      ), collapse = "; ")
    )
  }
