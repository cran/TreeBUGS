...
#' @param alpha Hyperprior for the shape parameters \eqn{\alpha} of the group-level beta distributions (in JAGS syntax). Default: Gamma distributions for \eqn{\alpha} and \eqn{\beta} with shape 1 and rate .1 (see\link{plotPrior}. A vector can be used to specify separate hyperpriors for each MPT parameter (the order of parameters is determined by the names of the vector or by the default order as shown by \code{\link{readEQN}} with \code{paramOrder = TRUE}). Originally, Smith and Batchelder (2008) used the "WinBUGS-zeros-trick" (available in TreeBUGS if \code{alpha="zero"} or \code{beta="zero"}), which approximates uniform priors on the group-level mean and SD (but often results convergence issues).
