...
plotPriorPost <- function(
  ...
  samples <- sampleHyperprior(fittedModel$mptInfo$hyperprior,
    M = M, S = S,
  ...
  for (s in 1:S) {
    ...
    plot(d.mean,
      main = paste0("Group mean of ", fittedModel$mptInfo$thetaUnique[s]),
      xlab = "Group mean", las = 1, ...
    )
    lines(prior.mean, col = "blue", lty = "dashed")
    abline(v = ci.mean, col = "red")
    plot(d.sd,
      main = paste0("Group SD of ", fittedModel$mptInfo$thetaUnique[s]),
      xlab = xlab.sd, las = 1, ...
    )
  ...
  if (fittedModel$mptInfo$model == "traitMPT" & S > 1) {
    ...
    for (s1 in 1:(S - 1)) {
      for (s2 in (s1 + 1):S) {
        ...
        abline(v = quantile(
          unlist(fittedModel$runjags$mcmc[, paste0("rho[", s1, ",", s2, "]")]),
          c((1 - ci) / 2, 1 - (1 - ci) / 2)
        ), col = "red")
      }
    }
